...
inner join produto
on produto.idmarca = marca.idMarca where produto.preco between 1000 and 3000 order by produto.preco desc;
...
inner join produto
on itens_pedido.idProduto = produto.idProduto where cliente.id = 1;

-- 9 - Listar o número do pedido e o nome clientes que possuem pedidos e também dos que não possuem pedidos.
select pedido.idPedido as IdPedido, cliente.nome as NomeCliente, pedido.data_pedido from cliente
left join pedido
...
select pedido.IdPedido as NumeroPedido, itens_pedido.valor_total, cliente.nome from itens_pedido
inner join pedido
on itens_pedido.idPedido = pedido.IdPedido
inner join cliente
